...
## [2026-09-25] 第 42 輪 — 每次掃描用自己的掃描器物件

- **問題**：`get_adapters()` 回傳全域單例，`scan()` 把 custom_rules／rulesets／_on_stage 寫在 self 上；在「寫入」到「讀取」之間（版本探測，數秒），另一個掃描可覆蓋 → 默默少跑自訂規則，或用到已刪除的規則檔。
- **修法**：`get_adapters()` 每次建立新實例；`ADAPTERS` 只當唯讀目錄。不改並行度（使用者選擇）。
- **驗證**：以 Event 讓 A 停在探測、B 用別的規則跑完，再放 A；舊程式碼失敗、新程式碼通過。pytest 377。
- **教訓**：原本的註解寫著「傳參數而不是讀 config，因為會並行」——意圖是對的，但把參數存到共用物件上等於沒傳。註解描述的保證，要有測試證明它成立。
...
- **判定**：使用者同意後，工具未完成 → 人工審查（原本 passed 的情況）。上線實測：原本會是「passed、0 項」的 nessus-report，現在是 manual_review，並指出 bearer 沒分析 app.js。
