...
## [2026-09-25] #018 — MCP 可掃描地端檔案（兩段式 ZIP 上傳）

- **使用者指令（提示詞原意）**：能否修改 SAST 專案讓 MCP 可以掃描地端檔案？網頁已可上傳 ZIP。使用者從三案中選定「A 兩段式上傳」。
- **對照既有決議**：調整 #014 第 4 點「MCP 沿用網頁表單同一套 git URL 驗證」——git URL 的限制**不變**；新增的是一條與網頁 ZIP 上傳對等的路徑，不給助理比網頁使用者更多能力（不開放本機路徑、不跳過 ZIP 解壓的 zip-slip／大小／檔數限制）。已向使用者說明並取得選擇。
- **決議回應**：
  1. 新 MCP 工具 `create_upload_scan`：回傳一次性、短時效（10 分鐘）的上傳票證，與呼叫者帳號綁定；每人同時未用票證上限 5 張。
  2. 新端點 `PUT /api/mcp/upload`：以 `Authorization: Bearer <票證>` 上傳 ZIP 原始內容；票證放標頭不放網址（避免進 nginx 存取日誌）；用過即失效；上傳時重新確認帳號仍有效；沿用 `MAX_UPLOAD_BYTES` 與既有 ZIP 解壓流程。
  3. 結果照舊用 `get_scan_result` 讀取（owner = 票證所屬使用者）。
- **理由**：MCP 工具參數是 JSON，不適合夾帶 ZIP；兩段式讓大檔走 HTTP 串流，且可掃未推送的程式碼。
- **影響範圍**：`app/mcp.py`、`app/main.py`、測試、README 中英。
- **後續動作**：實作完成；pytest 344/344；獨立複核無 Critical/High/Medium，Low 3 修 2（FIND-003 與網頁上傳共有，記為已知風險）。commit／push／重建映像部署屬不可逆動作，待使用者核准。

## [2026-09-25] #019 — 核准 commit／push／部署並重掃
...
## [2026-09-25] #020 — 查 bearer 少報根因，並檢查其他掃描器
- **使用者指令（提示詞原意）**：實作「查出 bearer 少報根因、讓『沒分析任何檔案』不再顯示 ok」，並確認其他掃描工具會不會有同樣問題。
- **對照既有決議**：延續 #019 記下的待辦；與第 15 輪「每個被要求的工具都回報 ok 才算數」同一條準則。
- **決議回應**：先以實驗重現並定位根因（不猜），再修；六個 adapter 逐一檢查「有跑但沒分析」的情形。
- **根因**：bearer 單檔超時略過、只寫 debug log、rc 0；在 2 核 VM 上 99 KB 的 JS 檔卡在期限邊緣，結果不穩定。git／ZIP 差異不是原因（三種配置單獨實測相同）。
- **其他掃描器**：實測 semgrep／trivy／osv（部分）／gitleaks 都會在略過或失敗時回報 ok；npm audit 正確。已全部改為 incomplete 或 error，並在容器內以真實工具驗證。
- **待使用者決定**：判定規則目前不看工具狀態（全部失敗也是 passed），屬第 17 輪使用者決議，未改動；push 與部署待核准。

## [2026-09-25] #021 — 判定規則納入工具狀態；推送並部署第 41 輪
- **使用者指令（提示詞原意）**：「都做」：(1) 推送 d49d845、跑 CI、部署到 .145；(2) 判定規則納入工具狀態。
...
## [2026-09-25] #022 — 每次掃描建立新的掃描器物件
- **使用者指令（提示詞原意）**：修正「掃描器物件被所有掃描共用，同時進行的掃描可能互相覆蓋自訂規則」；只做這一項（不改為輪流掃描）。
- **對照既有決議**：第 41 輪記錄的既有問題；不改並行度設定。
- **決議回應**：`get_adapters()` 每次回傳新的實例；`ADAPTERS` 清單只留作工具目錄（名稱、說明、版本探測）。
- **後續動作**：已實作並以可重現競態的測試驗證（舊碼失敗、新碼通過）；pytest 377。push／部署待使用者核准。
- **後續動作（#022 完成）**：33d55f4 推送、CI 綠燈、部署 .145 healthy；兩個並行 MCP 掃描皆正常完成且結果一致。
