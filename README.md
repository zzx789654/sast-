...
| 任一工具失敗、逾時或**未完整分析**（`incomplete`） | **需人員審查**（`manual_review`），並列出是哪個工具、哪些檔案沒分析到 |
| 只有「低」、參考資訊，或完全沒發現，**且每個工具都完整跑完** | **通過**（`passed`） |

「未完整」是指工具有跑、但它自己回報有檔案沒分析完（例如 bearer 單檔超時、trivy 讀不懂 lockfile）。這些工具在這種情況下都會正常結束，過去會被當成「0 個發現」而通過。「未安裝」與「不適用」（專案裡沒有它能掃的東西）不算缺口。
...
顯示每個容器的記憶體與 CPU 上限、**實際用量**、以及**主機總量**。
三者一起看，因為上限本身沒有意義——2GB 是寬裕還是綁手綁腳，
取決於另外兩個數字。

預設沒有任何上限，面板會標成 **無上限 ⚠**——那是個發現，不是空欄位。
沒有上限的容器可以吃光整台主機，而不只是限制自己，一個失控的掃描
就會把所有東西拖垮。

填入想要的規格，按**產生設定**，會得到一段 compose 設定：
...
貼進 `docker-compose.yml`，再執行 `./scripts/deploy.sh` 即可生效。

**這裡刻意「產生設定」而不是「直接套用」。** 容器其實可以透過掛載的
socket 呼叫 `/containers/update`——`:ro` 讓 socket 檔案唯讀，但**不限制
透過它送出的指令**（實測回 409 而非 403）。不用它的理由有二：一是那會
讓這個應用取得「修改主機上任何容器」的權限，被攻破時影響範圍大很多；
二是 compose 才是這些值的真相來源，繞過它改的設定，下次部署就會被還原，
你會以為設定不見了。

伺服器端會擋掉不合理的值：超過主機容量的上限、負數、非數字，
以及帶有 YAML 特殊字元的服務名稱。
